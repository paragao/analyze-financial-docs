...
def upload_object(request, response, name=None):
    ...
    #
    #if name is not None:
    #    new_object = '{}/{}-{}.json'.format(prefix, os.path.splitext(request["objectName"].lower())[0], name)
    #else: 
    #    new_object = '{}/{}.json'.format(prefix, os.path.splitext(request["objectName"].lower())[0])
    #
    #bucket = request["bucketName"]
